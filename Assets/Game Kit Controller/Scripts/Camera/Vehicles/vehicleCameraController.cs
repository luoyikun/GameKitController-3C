	...
	void FixedUpdate ()
	{
		...
		if (drivingVehicle) {
			...
			if (!isGamePaused && !cameraPaused) {
				...
				bool checkCameraInputResult = true;

				if (currentState.cameraFixed) {
					checkCameraInputResult = false;

					if (currentState.allowInputRotationOnCameraFixed) {
						...
						if (!checkCameraInputResult) {
							if (Mathf.Abs (currentState.pivotTransform.localEulerAngles.x) > 0.1f) { 
								...
								Quaternion.Slerp (currentState.pivotTransform.localRotation, Quaternion.identity, smoothCameraRotationSpeedVerticalThirdPerson * Time.deltaTime);
							}
						}
					}
					...
					if (!checkCameraInputResult) {
						...
						transform.eulerAngles = Vector3.MoveTowards (transform.eulerAngles, (transform.eulerAngles + transform.up * fixedRotationAmount), 
							Time.fixedDeltaTime * currentState.rotationDamping);
					}
				}
