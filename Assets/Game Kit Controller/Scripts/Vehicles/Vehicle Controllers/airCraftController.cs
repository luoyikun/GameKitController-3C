...
public class airCraftController : vehicleController
{
	...
	public override void Start ()
	{
		...
		foreach (ParticleSystem child in components) {
			...
			if (newExhaust.particles.gameObject.activeSelf) {
				newExhaust.particles.gameObject.SetActive (false);
			}

			newExhaust.particlesActive = false;
...
//					}
	...
	void updateVehicleControllerState ()
	{
		if (driving) {
			...
			if (Mathf.Abs (horizontalAxis) <= 0.1f) {
				rightJoystickAngle.x = Mathf.Lerp (rightJoystickAngle.x, 0, currentDeltatime * settings.joysticksSensitivity);
			}

			rightJoystickAngle.y += verticalAxis * settings.joysticksSensitivity;

			if (Mathf.Abs (verticalAxis) <= 0.1f) {
				rightJoystickAngle.y = Mathf.Lerp (rightJoystickAngle.y, 0, currentDeltatime * settings.joysticksSensitivity);
			}
			...
			if (Mathf.Abs (rollInput) <= 0.1f) {
				leftJoystickAngle.x = Mathf.Lerp (leftJoystickAngle.x, 0, currentDeltatime * settings.joysticksSensitivity);
			}
			...
			if (usingBoost) {
				...
				leftJoystickAngle.y = Mathf.Lerp (leftJoystickAngle.y, 0, currentDeltatime * settings.joysticksSensitivity);
			}
			...
			if (enginesOn) {
				...
				vehicleParts.engineAudio.pitch = Mathf.Lerp (vehicleParts.engineAudio.pitch, 0, currentDeltatime * 2);
				vehicleParts.windAudio.pitch = Mathf.Lerp (vehicleParts.windAudio.pitch, 0, currentDeltatime * 2);
			}
		}
	}
		...
		if (driving) {
			if (Physics.Raycast (vehicleParts.landMarkRaycasPosition.position, -transform.up, out hit, settings.distanceToLand, settings.layer)) {
				...
				if (settings.useLandMark) {
					...
					vehicleParts.landMark.transform.position = hit.point + 0.2f * hit.normal;
					...
					vehicleParts.landMark.transform.GetChild (0).Rotate (0, 100 * currentDeltatime, 0);
				}
			} else {
				canLand = false;
			...
			if (enginesOn) {
				...
				if (realisticMovements) {
					if (settings.canRoll) {
						rollInput = Mathf.Lerp (rollInput, rollDirection, currentDeltatime * rollForce);
					}

					if (vehicleControllerSettings.canUseBoost) {
						if (usingBoost) {
							currentBoost = Mathf.Lerp (currentBoost, boostMultiplier, boostAccelerationSpeed * currentDeltatime);
						} else {
							currentBoost = Mathf.Lerp (currentBoost, 0, boostAccelerationSpeed * currentDeltatime);
						}
					}

					ThrottleInput = brakeActive ? -1 : 1;
					...
					Throttle = Mathf.Clamp01 (Throttle + ThrottleInput * currentDeltatime * throttleChangeSpeed);
					...
					mainRigidbody.drag = (brakeActive ? (originalDrag + extraDrag) * brakesForce : originalDrag + extraDrag);
					...
					if (mainRigidbody.velocity.magnitude > 0) {
						...
						Vector3 newVelocity = Vector3.Lerp (mainRigidbody.velocity, ForwardSpeed * transform.forward, m_AeroFactor * ForwardSpeed * aeroDynamicForce * currentDeltatime);
						mainRigidbody.velocity = newVelocity;

						if (playerMovingOn3dWorld) {
							// also rotate the plane towards the direction of movement - this should be a very small effect, but means the plane ends up
							// pointing downwards in a stall
							mainRigidbody.rotation = Quaternion.Slerp (mainRigidbody.rotation, Quaternion.LookRotation (mainRigidbody.velocity, transform.up), aeroDynamicForce * currentDeltatime);
						}
					}
					...
					if (settings.canRoll) {
						rollInput = Mathf.Lerp (rollInput, rollDirection, currentDeltatime * nonRealisticSettings.rollForce);
					}
					...
					if (vehicleControllerSettings.canUseBoost) {
						if (usingBoost) {
							currentVelocity = Mathf.Lerp (currentVelocity, nonRealisticSettings.maxBoostSpeed, nonRealisticSettings.boostAccelerationSpeed * currentDeltatime);
						} else if (brakeActive) {
							currentVelocity = Mathf.Lerp (currentVelocity, nonRealisticSettings.brakeSpeed, nonRealisticSettings.boostAccelerationSpeed * currentDeltatime);
						} else {
							currentVelocity = Mathf.Lerp (currentVelocity, nonRealisticSettings.maxForwardSpeed, nonRealisticSettings.boostAccelerationSpeed * currentDeltatime);
						}
					}
					...
					currentTorque *= currentDeltatime * mainRigidbody.mass;

					mainRigidbody.AddRelativeTorque (mainRigidbody.mass * currentTorque);

					airCraftVelocity = Vector3.MoveTowards (airCraftVelocity, currentVelocity * transform.forward, currentDeltatime * accelerationSpeed);

					mainRigidbody.velocity = airCraftVelocity;

					EnginePower = Mathf.Lerp (EnginePower, mainRigidbody.velocity.magnitude / 10, currentDeltatime * 5);
				}
			} 
		}

		if (mainRigidbody.velocity.magnitude != 0) {
			int exhaustParticlesCount = exhaustParticles.Count;

			for (int i = 0; i < exhaustParticlesCount; i++) {
				...
				exhaustParticles [i].currentSpeed = Mathf.Lerp (exhaustParticles [i].currentSpeed, paritclesSpeed, currentDeltatime * 5);
		...
		if (!enginesOn) {
			if (mainVehicleGravityControl.OnGround) {
				...
				for (int i = 0; i < exhaustParticles.Count; i++) {
					if (!exhaustParticles [i].particlesActive) {
							
						if (!exhaustParticles [i].particles.gameObject.activeSelf) {
						...
						exhaustParticles [i].particlesActive = true;
	...
	{
		if (state) {
			mainRigidbody.AddForce ((settings.extraTakeOffForce * mainRigidbody.mass) * transform.up, ForceMode.Impulse);
			...
			if (canLand) {
				for (int i = 0; i < exhaustParticles.Count; i++) {
					if (exhaustParticles [i].particlesActive) {
						...
						exhaustParticles [i].particlesActive = false;
					}
				}
		...
		for (int i = 0; i < boostingParticles.Count; i++) {
			if (usingBoost) {
				if (!boostingParticles [i].isPlaying) {
					if (boostingParticles [i].gameObject.activeSelf) {
