...
public class motorBikeController : vehicleController
{
	...
	public override void Start ()
	{
		...
		engineStartAudioLocated = otherCarParts.engineStartAudio != null;

		if (useStandAnimation) {
			if (mainAnimation == null) {
				useStandAnimation = false;
			}
		}
		...
		if (otherCarParts.boostParticles != null) {
	...
	public override void vehicleUpdate ()
	{
		...
		verticalLean = 
			Mathf.Clamp (Mathf.Lerp (verticalLean, transform.InverseTransformDirection (currentAngularVelocity).x * settings.chassisLean.y, 
			currentDeltatime * 5), -settings.chassisLeanLimit.y, settings.chassisLeanLimit.y);
		...
		horizontalLean = 
			Mathf.Clamp (Mathf.Lerp (horizontalLean, (transform.InverseTransformDirection (currentAngularVelocity).y * normalizedLeanAngle) *
		settings.chassisLean.x, currentDeltatime * 3), -settings.chassisLeanLimit.x, settings.chassisLeanLimit.x);
	...
	void FixedUpdate ()
	{
		...
		if (!anyOnGround && settings.preserveDirectionWhileInAir && currentSpeed > 5) {
			...
			if (velocityDirection > -20) {
				...
				transform.eulerAngles -= (angleX * currentDeltatime) * transform.InverseTransformDirection (transform.right);
			}
		}
		...
		//set the current RPM
		currentRPM = 
			Mathf.Clamp ((((Mathf.Abs ((frontWheel.wheelCollider.rpm + rearWheel.wheelCollider.rpm)) * settings.gearShiftRate) + settings.minRPM)) /
		...
		if (!vehicleDestroyed) {
			otherCarParts.engineAudio.volume = Mathf.Lerp (otherCarParts.engineAudio.volume, Mathf.Clamp (motorInput, 0.35f, 0.85f), currentDeltatime * 5);

			otherCarParts.engineAudio.pitch = 
				Mathf.Lerp (otherCarParts.engineAudio.pitch, Mathf.Lerp (1, 2, (currentRPM - (settings.minRPM / 1.5f)) / (settings.maxRPM + settings.minRPM)), currentDeltatime * 5);
		}

		if (engineStartAudioLocated) {
			if (otherCarParts.engineStartAudio.volume > 0) {
				otherCarParts.engineStartAudio.volume -= currentDeltatime / 5;
			}
		}
		...
		if (isTurnedOn) {
			...
			int normalExhaustCount = otherCarParts.normalExhaust.Count;

			for (i = 0; i < normalExhaustCount; i++) {
			...
			int heavyExhaustCount = otherCarParts.heavyExhaust.Count;

			for (i = 0; i < heavyExhaustCount; i++) {
		...
		torqueVector = transform.InverseTransformDirection (torqueVector).z * transform.forward;
	...
	public void checkDriverFootPosition ()
	{
		if (lowSpeedDetected != previoslyLowSpeedDetected) {
			...
			if (lowSpeedDetected) {
				...
				Vector3 raycastPositionTransformPosition = Vector3.zero;

				if (useRightFootForGround) {
					raycastPositionTransformPosition = rightRaycastPositionToFeet.position;
				} else {
					raycastPositionTransformPosition = leftRaycastPositionToFeet.position;
				}

				if (Physics.Raycast (raycastPositionTransformPosition, -currentNormal, out hit, 20, settings.layer)) {
					raycastPositionFoundForFeet = hit.point;

					raycastPositionFoundForFeet += 
						footOnGroundPositionOffset.x * transform.right +
					footOnGroundPositionOffset.y * hit.normal +
					footOnGroundPositionOffset.z * transform.forward;
