...
public class inventoryQuickAccessSlotsSystem : MonoBehaviour
{
	...
	public bool keepQuickAccessSlotsHiddenOnMainScreen;
	...
	public bool changeAlwaysToWeaponWithoutDoubleSelection;
	...
	public bool useEventsOnChangeWeaponsWithKeysActive;
	public eventParameters.eventToCallWithBool eventsOnChangeWeaponsWithKeysActive;
	...
	public void initializeQuickAccessSlots ()
	{
		...
		if (useInventoryQuickAccessSlots) {
			...
			setInventoryQuickAccessSlotsActiveState (false);
		}
	}
			...
			for (int i = 0; i < touchCount; i++) {
				...
				if (currentTouch.phase == TouchPhase.Began && !touching) {
					...
					foreach (RaycastResult r in captureRaycastResults) {
						...
						inventorySlotOptionsButtons currentInventorySlotOptionsButtons = r.gameObject.GetComponent<inventorySlotOptionsButtons> ();

						if (currentInventorySlotOptionsButtons != null) {
							if (showDebugPrint) {
								print ("InventorySlotOptionsButtons found, ignoring the rest of UI elements below");
							}

							return;
						}

						simpleInventorySlotOptionsButton currentsimpleInventorySlotOptionsButton = r.gameObject.GetComponent<simpleInventorySlotOptionsButton> ();

						if (currentsimpleInventorySlotOptionsButton != null) {
							if (showDebugPrint) {
								print ("Simple InventorySlotOptionsButton found, ignoring the rest of UI elements below");
							}
					...
					for (int j = 0; j < inventoryListCount; j++) {
						if (!objectsSwaped) {
							...
							if (currentInventoryInfo.button == currentInventoryMenuIconElement.button) {
								if (showDebugPrint) {
									print ("slot wih object " + currentSlotToMoveInventoryObject.Name + " dragged from inventory grid dropped into " + currentInventoryInfo.Name);
								}

								if (currentSlotToMoveInventoryObject != currentInventoryInfo) {
									int slotToMoveIndex1 = mainInventoryManager.getInventoryObjectIndexByInventoryInfo (currentSlotToMoveInventoryObject);

									int slotToMoveIndex2 = j;
								
									if (slotToMoveIndex1 != -1 && slotToMoveIndex2 != -1) {
										...
										if (showDebugPrint) {
											print ("item 1 or 2 not found when trying to swap slots on inventory grid, cancelling swap");
										}
									}
...
//						}
	...
	public void selectFirstMeleeWeaponAvailable ()
	{
		for (int j = 0; j < quickAccessSlotInfoListCount; j++) {
			if (currentQuickAccessList [j].slotActive &&
			    currentQuickAccessList [j].inventoryInfoAssigned.isMeleeWeapon) {
	...
	public void selectFirstFireWeaponAvailable ()
	{
		for (int j = 0; j < quickAccessSlotInfoListCount; j++) {
			if (currentQuickAccessList [j].slotActive &&
			    currentQuickAccessList [j].inventoryInfoAssigned.isWeapon &&
			    !currentQuickAccessList [j].inventoryInfoAssigned.isMeleeWeapon) {
	...
	public void selectFirstGeneralWeaponAvailable ()
	{
		for (int j = 0; j < quickAccessSlotInfoListCount; j++) {
			if (currentQuickAccessList [j].slotActive &&
			    currentQuickAccessList [j].inventoryInfoAssigned.isWeapon) {
	...
	bool changeToWeaponWithoutDoubleSelection;

	public void setOverrideChangeToWeaponWithoutDoubleSelectionState (bool state)
	{
		changeToWeaponWithoutDoubleSelection = state;
	}
			...
			if (currentSlotInfo.slotActive) {
				...
				if (inventoryObjectInfo.isWeapon || inventoryObjectInfo.isMeleeWeapon || inventoryObjectInfo.isMeleeShield) {
					...
					if (!isMeleeWeapon) {
						if (mainPlayerWeaponsManager.isWeaponsModeActive ()) {
							...
							if (selectWeaponResult) {
								bool changeToWeaponWithoutDoubleSelectionResult = false;

								if (changeToWeaponWithoutDoubleSelection) {
									changeToWeaponWithoutDoubleSelectionResult = true;

									changeToWeaponWithoutDoubleSelection = false;
								}

								if (changeAlwaysToWeaponWithoutDoubleSelection) {
									changeToWeaponWithoutDoubleSelectionResult = true;
								}

								if (showDebugPrint) {
									print ("check Weapon To Select On Quick Access Slots " + inventoryObjectInfo.Name + " " + changeToWeaponWithoutDoubleSelectionResult);
								}

								mainPlayerWeaponsManager.checkWeaponToSelectOnQuickAccessSlots (inventoryObjectInfo.Name, changeToWeaponWithoutDoubleSelectionResult);
							}
					...
					if (inventoryObjectInfo.canBeEquiped) {
						...
						mainInventoryManager.useEquippedObjectAction (inventoryObjectInfo.Name, inventoryObjectInfo);
					} else {
						if (showDebugPrint) {
	...
	public void inputSelectNextOrPreviousQuickAccessSlot (bool state)
	{
		...
		if (useEventsOnChangeWeaponsWithKeysActive) {
			eventsOnChangeWeaponsWithKeysActive.Invoke (state);
	...
	public void updateQuickAccessSlotInfo (int slotIndex, inventoryInfo currentSlotToMove, inventoryQuickAccessSlotElement.quickAccessSlotInfo slotToUnEquip, playerWeaponSystem secondaryWeaponToEquip)
	{
		...
		if (slotIndex > -1 && slotIndex < quickAccessSlotInfoListCount && currentSlotToMove != null) {
			...
			if (showDebugPrint) {
				print ("updateQuickAccessSlotInfo for inventory object " + currentSlotToMove.Name);
			}
		...
		if (!slotFound && slotToUnEquip != null) {
			if (activatingDualWeaponSlot) {
				...
				if (showDebugPrint) {
					print ("unequipping slot " + slotToUnEquip.Name);
				}
	...
	IEnumerator showQuickAccessSlotsParentWhenSlotSelectedCoroutine (int slotIndex)
	{
		if (slotIndex >= 0 && quickAccessSlotInfoListCount >= slotIndex) {
			...
			bool isTurnBasedCombatActionActive = mainPlayerWeaponsManager.playerManager.isTurnBasedCombatActionActive ();
			...
			if (!mainInventoryManager.inventoryOpened) {
				if (!keepQuickAccessSlotsHiddenOnMainScreen && !isTurnBasedCombatActionActive) {
			...
			if (!showQuickAccessSlotsAlways || isTurnBasedCombatActionActive) {
				moveQuickAccessSlotsToInventory ();
			}
	...
	public void setKeepQuickAccessSlotsHiddenOnMainScreenState (bool state)
	{
		keepQuickAccessSlotsHiddenOnMainScreen = state;
	}
	...
	public void setKeepQuickAccessSlotsHiddenOnMainScreenStateFromEditor (bool state)
	{
		setKeepQuickAccessSlotsHiddenOnMainScreenState (state);
