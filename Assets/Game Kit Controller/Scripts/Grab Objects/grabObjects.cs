		...
		if (objectHeld != null && !carryingPhysicalObject) {
			...
			if (currentDistanceToGrabbedObject > currentMaxDistanceHeld && grabbed && regularObject) {
				...
				if (regularObject) {
					...
					objectHeldRigidbody.velocity = holdSpeed * (nextObjectHeldPosition - currentObjectHeldPosition);
...
//			print ("removed " + objectToGrab.name);
	...
	void grabPhysicalObjectDirectly (grabPhysicalObjectSystem.grabPhysicalObjectInfo newGrabPhysicalObjectInfo)
	{
		...
		for (int i = 0; i < componentsLength; i++) {
			...
			playerControllerManager.setIgnoreCollisionOnExternalColliderOnlyWithExtraColliderList (child, true);
		}
