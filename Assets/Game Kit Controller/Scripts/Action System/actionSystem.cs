...
public class actionSystem : MonoBehaviour
{
    ...
    Vector3 objectToSetParentOriginalPosition;
    Vector3 objectToSetParentOriginalRotation;

    Transform objectToSetParentOriginalParent;

    Transform currentObjectToSetParent;


    public void setObjectToSetParentInfo (Transform currentObject, Vector3 position, Vector3 rotation, Transform originalParent)
    {
        currentObjectToSetParent = currentObject;

        objectToSetParentOriginalPosition = position;
        objectToSetParentOriginalRotation = rotation;

        objectToSetParentOriginalParent = originalParent;
    }
    public void checkResetObjectPositionAndParentAfterAction ()
    {
        if (currentObjectToSetParent != null) {
            currentObjectToSetParent.SetParent (objectToSetParentOriginalParent);

            currentObjectToSetParent.localPosition = objectToSetParentOriginalPosition;
            currentObjectToSetParent.localEulerAngles = objectToSetParentOriginalRotation;
        }
    }
                                        ...
                                        bool useActionSystemID, int actionSystemID, string actionSystemAnimationName)
    {
        ...
        gameObject.name = actionSystemName;
    ...
    public void replaceAnimationAction ()
    {
        ...
        if (actionInfoList.Count > 0) {
            ...
            if (actionNameToReplace != "") {
                ...
                for (int i = 0; i < actionInfoList.Count; i++) {
                    if (!actionFound && (actionInfoList [i].actionName.Equals (actionNameToReplace) || actionInfoList [i].Name.Equals (actionNameToReplace))) {
                        actionFound = true;
            ...
            resetActionSystemReplaceInfo ();
        }
#endif
    }

    void resetActionSystemReplaceInfo ()
    {
    ...
    public void enableOrDisableAutoUseActionOnTriggerByAI (bool state)
    {
        if (!Application.isPlaying) {
            if (actionInfoList.Count > 0) {
                actionInfo currentactionInfo = actionInfoList [0];

                currentactionInfo.pauseAIOnActionStart = state;
                currentactionInfo.resumeAIOnActionEnd = state;
                currentactionInfo.checkUseMinAngleToActivateAction = state;
                currentactionInfo.checkUseMinDistanceToActivateAction = state;

                eventTriggerSystem currentEventTriggerSystem = GetComponent<eventTriggerSystem> ();

                if (currentEventTriggerSystem != null) {
                    currentEventTriggerSystem.setUseTagListState (state);

                    if (state) {
                        currentEventTriggerSystem.addElementToTagList (GKC_Utils.getPlayerTag ());
                        currentEventTriggerSystem.addElementToTagList (GKC_Utils.getFriendTag ());
                        currentEventTriggerSystem.addElementToTagList (GKC_Utils.getEnemyTag ());
                    } else {
                        currentEventTriggerSystem.removeElementToTagList (GKC_Utils.getFriendTag ());
                        currentEventTriggerSystem.removeElementToTagList (GKC_Utils.getEnemyTag ());
                    }
                }
    ...
    public class actionInfo
    {
        ...
        public bool setCustomHeadTrackTargetToLook;
        public Transform customHeadTrackTargetToLook;

        public bool useCustomHeadTrackTolook;
        public string customHeadTrackToLookName;
        ...
        public bool usePositionToAdjustPlayerOnlyOnFBA;
        ...
        public bool ignoreSetPlayerFacingDirectionOnFBA;
        ...
        public bool resetObjectPositionAndParentAfterAction = true;
        ...
        [TextArea (1, 3)]
        public string useRaycastToAdjustMatchTransformExplanation = "This is the option to adjust the matchTargetTransform " +
                                                                                      "transform to the ground, used to match the player position " +
                                                                                      "to the animation position.";

        [TextArea (1, 3)]
        public string useRaycastToAdjustTargetTransformExplanation = "This is the option to adjust the playerWalkTarget " +
                                                                                       "transform to the ground, used to make the player to walk to " +
                                                                                       "a certain position during the action.";

        [TextArea (1, 3)]
        public string useRaycastToAdjustPositionToAdjustPlayerExplanation = "This is the option to adjust the positionToAdjustPlayer " +
                                                                                              "transform to the ground, used to move the player to the " +
                                                                                              "exact zone to play the animation.";
        ...
        public bool checkUseMinDistanceToActivateAction;
