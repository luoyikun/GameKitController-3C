...
public class meleeCombatThrowReturnWeaponSystem : MonoBehaviour
{
    ...
    public bool useHoldToThrowObjectOnAllWeaponsOn2_5dEnabled = true;

    public bool useHoldToThrowObjectOnAllWeaponsOn3dLockedViewEnabled = true;

    public bool calculateThrowDirectionBeforeThrowAnimationEnabled;
    ...
    public bool pauseDamageReactionDuringWeaponHoldEnabled;
    ...
    Vector3 currentThrowObjectDirection;

    RaycastHit currrenThrowObjectHit;

    bool calculateThrowDirectionBeforeThrowAnimationEnabledResult;

    bool surfaceFoundOnFirstRaycastResult;
    ...
    bool damageReactionStateChanged;
    ...
    void setPressDownOrUpOnThrowObject (bool state)
    {
        ...
        bool aimingThrowMeleeWeaponOn2_5dResult = false;

        if (!currentMeleeWeapon.holdToThrowObjectEnabled) {
            bool cancelHoldResult = true;

            if (!isPlayerMovingOn3dWorld && useHoldToThrowObjectOnAllWeaponsOn2_5dEnabled) {
                cancelHoldResult = false;

                aimingThrowMeleeWeaponOn2_5dResult = true;
            }

            if (isPlayerMovingOn3dWorld && !isCameraTypeFree && useHoldToThrowObjectOnAllWeaponsOn3dLockedViewEnabled) {
                cancelHoldResult = false;
        ...
        if (objectThrown) {
            ...
            setDamageReactionPausedState (false);
        ...
        if (state) {
            ...
            if (currentMeleeWeapon.locateObjectstToTrackOnHoldEnabled || aimingThrowMeleeWeaponOn2_5dResult) {
    ...
    void setDamageReactionPausedState (bool state)
    {
        if (pauseDamageReactionDuringWeaponHoldEnabled) {
            if (state) {
                if (!damageReactionStateChanged) {
                    mainPlayerController.setDamageReactionPausedState (true);

                    damageReactionStateChanged = true;
                }
            } else {
                if (damageReactionStateChanged) {
                    mainPlayerController.setDamageReactionPausedState (false);

                    damageReactionStateChanged = false;
                }
            }
        }
    }
    ...
    public void stopUpdateLocateTargetsCoroutine ()
    {
        ...
        if (!mainPlayerController.isPlayerMovingOn3dWorld () || !mainPlayerCamera.isCameraTypeFree ()) {
            ...
            if (Time.time > lastTimeThrowObjectInputPressed + 0.5f) {
                ...
                if (Physics.Raycast (mainCameraTransform.position, mainCameraTransform.forward, out locateTargetHit,
                    ...
                    if (locateTargetHit.transform.gameObject != playerControllerGameObject) {
                        ...
                        if (Physics.Raycast (locateTargetHit.point + 0.2f * mainCameraTransform.forward, mainCameraTransform.forward, out locateTargetHit,
                                currentMeleeWeapon.maxRaycastDistanceToLocateObjectToTrack, currentMeleeWeapon.layerToLocateObjectsToTrack)) {
                            surfaceFound = true;
                        }
                    }
    ...
    IEnumerator throwObjectCoroutine ()
    {
        ...
        bool isFullBodyAwarenessActive = mainPlayerController.isFullBodyAwarenessActive ();

        calculateThrowDirectionBeforeThrowAnimationEnabledResult = calculateThrowDirectionBeforeThrowAnimationEnabled || isFullBodyAwarenessActive;

        if (calculateThrowDirectionBeforeThrowAnimationEnabledResult) {
            calculateThrowDirectionInfo ();
        }
    ...
    void calculateThrowDirectionInfo ()
    {
        currentThrowObjectDirection = mainCameraTransform.forward;

        surfaceFoundOnFirstRaycastResult = false;

        if (Physics.Raycast (mainCameraTransform.position, mainCameraTransform.TransformDirection (Vector3.forward), out currrenThrowObjectHit, Mathf.Infinity, throwObjectsLayerToCheck)) {
             surfaceFoundOnFirstRaycastResult = true;
        }
    }
    ...
    IEnumerator continueThrowObjectCoroutine ()
    {
        ...
        if (currentMeleeWeapon.delayToThrowObject > 0) {
            WaitForSeconds delay = new WaitForSeconds (currentMeleeWeapon.delayToThrowObject);
        ...
        if (surfaceFoundOnFirstRaycastResult) {
            Vector3 heading = currrenThrowObjectHit.point - currentObjectRigidbody.transform.position;

            float distance = heading.magnitude;
            currentThrowObjectDirection = heading / distance;

            currentThrowObjectDirection.Normalize ();
        }

        Vector3 throwObjectDirection = currentThrowObjectDirection;

        RaycastHit hit = currrenThrowObjectHit;

        Vector3 forceDirection = (currentObjectRigidbody.mass * currentMeleeWeapon.throwSpeed) * throwObjectDirection;
        ...
        bool useDownForceOnWeaponTravel = currentMeleeWeapon.useDownForceOnWeaponTravel;

        float downForceOnWeaponTravel = currentMeleeWeapon.downForceOnWeaponTravel;
        ...
        while (!targetReachedOnThrow) {
            ...
            if (throwObjectWithRotation) {
                ...
                if (rotateObjectActive) {
                    if (!currentMeleeWeapon.rotateObjectToThrowDirection) {
                        objectRotationPoint.Rotate ((currentMeleeWeapon.throwObjectRotationSpeed * Time.deltaTime) * Vector3.right);
                    }
                }
            } else {
            ...
            if (useDownForceOnWeaponTravel) {
                currentObjectRigidbody.AddForce (downForceOnWeaponTravel * (-Vector3.up));
            }
                                ...
                                if (hits [i].collider.attachedRigidbody != null) {
                                    ...
                                    if (!detectedObjectsOnThrow.Contains (currentObject)) {
                                        ...
                                        if (currentMeleeWeapon.pushCharactersDetectedOnIgnoreRigidbodiesOnThrowWeapon) {
                                            ...
                                            Vector3 pushCharacterDirection = newPushForce * throwObjectDirection;
                    ...
                    if (distanceToTarget < 1 || objectLocatedWithRaycastResult) {
                        if (currentObjectToFollowFromHoldThrowWeapon != null) {
                            ...
                            if (currentMeleeWeapon.pushCharactersDetectedOnIgnoreRigidbodiesOnReturnWeapon) {
                                ...
                                Vector3 pushCharacterDirection = newPushForce * currentDirection;
                        ...
                        if (applyDamage.objectCanBeDamaged (surfaceDetecetedOnObjectThrown)) {
                            if (!applyDamage.checkIfDead (surfaceDetecetedOnObjectThrown)) {
                                isAttachedToCharacter = applyDamage.attachObjectToSurfaceFound (surfaceDetecetedOnObjectThrown.transform, currentGrabbedObjectTransform,
                                    currentMeleeWeapon.mainDamagePositionTransform.position, false, Vector3.zero);
        ...
        if (objectRotationPointParent.localEulerAngles == Vector3.zero) {
            currentGrabbedObjectTransform.localPosition = (-1) * originalPosition;
        } else {
    ...
    IEnumerator returnObjectCoroutine ()
    {
        ...
        WaitForSeconds delay = new WaitForSeconds (currentMeleeWeapon.delayToReturnObject);
        ...
        Vector3 handLocalPosition = referencePositionThirdPerson.localPosition;
        Quaternion handLocalRotation = referencePositionThirdPerson.localRotation;

        if (mainGrabbedObjectMeleeAttackSystem.isUseCustomGrabbedWeaponReferencePositionActive ()) {
            if (mainGrabbedObjectMeleeAttackSystem.isUseGrabbedWeaponReferenceValuesActive ()) {
                handLocalPosition = mainGrabbedObjectMeleeAttackSystem.getGrabbedWeaponReferenceValuesPosition ();
                handLocalRotation = Quaternion.Euler (mainGrabbedObjectMeleeAttackSystem.getGrabbedWeaponReferenceValuesEuler ());
            } else {
                referencePositionThirdPerson = mainGrabbedObjectMeleeAttackSystem.getCustomGrabbedWeaponReferencePosition ();

                handLocalPosition = referencePositionThirdPerson.localPosition;
                handLocalRotation = referencePositionThirdPerson.localRotation;
            }
        }

        handPositionReference.localRotation = handLocalRotation;
        handPositionReference.localPosition = handLocalPosition;
        ...
        while (!targetReached) {
            ...
            if (resetObjectRotationPointRotation) {
                ...
                if (returnObjectWithRotation) {
                    objectRotationPoint.Rotate ((currentMeleeWeapon.returnObjectRotationSpeed * Time.deltaTime) * Vector3.right);
                } else {
        ...
        currentGrabbedObjectTransform.localRotation = handLocalRotation;
        currentGrabbedObjectTransform.localPosition = handLocalPosition;
