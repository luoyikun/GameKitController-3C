...
public class ragdollActivator : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        if (!usedByAI) {
            if (playerState == healthState.dead) {
                ...
                if (Physics.Raycast (raycastPosition + Vector3.up, -Vector3.up, out hit, 2.7f, layer) &&
                    (!currentHipsRigidbodyLocated || currentHipsRigidbody.velocity.magnitude < maxVelocityToGetUp)) {
        ...
        if (playerState == healthState.fallen) {
            if (sedateActive) {
                ...
                //check if the player is on the ground, so he can get up
                if (!currentHipsRigidbodyLocated || currentHipsRigidbody.velocity.magnitude < maxVelocityToGetUp) {
                    onGround = true;
                } else {
                    onGround = false;
                }
            ...
            if (currentRigidbodyPart != null) {
                ...
                if (applyForceOnRagdollActive) {
                    if (!state) {
                        ...
                        if (pushAllBodyParts) {
                            currentRigidbodyPart.velocity = (extraForceOnRagdoll / 2) * damageDirection;
                        } else {
    ...
    {
        pushFullCharacter (speedMultiplier * mainRigidbody.velocity);
    }
