...
public class playerController : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        //(!fullBodyAwarenessActive && crouching && canCrouchWhenUsingWeaponsOnThirdPerson && playerUsingWeapons);
        ...
        //		if (usedByAI) {
        //			print (hasToLookInCameraDirection () + " " + checkLookCameraDirectionResult + " " + strafeModeActive);
        //		}
        ...
        if (!usedByAI && isPlayerMovingOn2_5dWorld ()) {
            ...
            if (playerIsAiming) {
                ...
                if (!useSmoothPlayerRotationAxisOn2_5dEnabled) {
                    if (rotatingPlayerIn2_5dToRLeft) {
                        playerTransform.rotation = Quaternion.LookRotation (-currentLockedCameraTransform.right);
    ...
    bool isColliderPartOfCharacter (Collider newCollider)
    {
        if (newCollider == null) {
            return false;
        }
    ...
    public void setTimeOnAirAnimatorIDValue ()
    {
        ...
        //		print (lastJumpTime + " " + lastTimeAir);

        //		if (lastJumpTime > lastTimeAir) {
        //			newValue = Time.time - lastTimeAir;
        //		} else {
        //			newValue = Time.time - lastJumpTime;
        //		}
        ...
        //		print ("UPDATE TIME ON AIR " + newValue + " " + Time.time + " " + lastTimeOnAirAnimator);
    }
        ...
        if (!gravityPowerActive && !usingJetpack && !flyModeActive && !swimModeActive && !externalControlBehaviorForAirTypeActive) {
            ...
            if (playerTransform.InverseTransformDirection (mainRigidbody.velocity).y < downSpeedValue) {
                ...
                if (useSphereRaycastForGroundDetection) {
                    ...
                    if (!groundDetected) {
                        ...
                        if (Physics.SphereCast (sphereRaycastRay, sphereCastRadius, out sphereRaycastHit, maxDistanceSphereCast, layer)) {
                            ...
                            //							if (groundDetected) {
                            //								print ("ground found by sphere cast gap");
                            //							}
                        }
                    }
                ...
                if (groundDetected) {
                    ...
                    //					print (hitAngle + " " + GKC_Utils.getAngle (currentNormal, mainGroundHit.normal));
