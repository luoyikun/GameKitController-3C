		...
		if (movementSystemActive && !movementPausedDebug) {
			if (!adjustingPlayerToMovement) {
				...
				if (movingUp || movingDown) {
					playerTransform.position = Vector3.MoveTowards (playerTransform.position, targetPosition, Time.fixedDeltaTime * currentSpeed);
				}
				...
				if (Mathf.Abs (targetRotation) > 0.001f) {
					if (isFullBodyAwarenessActive) {
						playerCameraTransform.Rotate (0, targetRotation * 4, 0);
					} else {
	...
	public void setJumpActive (Vector3 newImpulseOnJumpAmount)
	{
		if (movementSystemActive) {
			...
			if (isFirstPersonActive) {
				float angleWithCamera = Vector3.SignedAngle (playerTransform.forward, playerCameraTransform.forward, playerTransform.up);

				if (Mathf.Abs (angleWithCamera) > 45) {
					currentDirectionToRotatePlayer = playerCameraTransform.forward;
				}
			} 
...
//						print (ABSAngle + " " + i);
	...
	public void setMovementSystemActivestate (bool state)
	{
		...
		if (state) {
			...
			if (currentExternalControllerBehavior != null && currentExternalControllerBehavior != this) {
				...
				if (mainPlayerController.isAvoidListOfExternalControllerStatesActive ()) {
					if (mainPlayerController.checkIfInsideExternalControllerToAvoidList (behaviorName)) {
						if (showDebugPrint) {
							print ("external controller " + behaviorName + " is on the current list of traversal movements to ignore, cancelling");
						}
	...
	IEnumerator adjustPlayerToMovementOnStartCoroutine (Vector3 initialPosition, Vector3 movementDirection)
	{
		...
		Vector3 currentPlayerForward = Vector3.Cross (objectToRotate.right, movementDirection);
		...
		if (!isPlayerMovingOn3dWorld) {
			...
			if (mainPlayerController.isPlayerLookingAtRightOn2_5d ()) {
				playerTargetRotation = Quaternion.LookRotation (currentLockedCameraTransform.right);
			} else {
				playerTargetRotation = Quaternion.LookRotation (-currentLockedCameraTransform.right);
			}
		}
