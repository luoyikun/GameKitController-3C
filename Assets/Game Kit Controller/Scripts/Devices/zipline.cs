...
public class zipline : MonoBehaviour
{
	...
	public void changeZiplineState (bool state)
	{
		...
		if (state) {
			...
			playerControllerManager.setPlayerAndCameraParent (movingTransform);
	...
	IEnumerator ziplineMovment ()
	{
		...
		float dist = GKC_Utils.distance (movingTransform.transform.position, movingTransformTargetPosition);
		...
		while (!targetReached) {
			...
			movingTransform.transform.position = 
				Vector3.MoveTowards (movingTransform.transform.position, movingTransformTargetPosition, t);
