...
public class lockedCameraSystem : MonoBehaviour
{
    ...
    [TextArea (5, 10)]
    public string cameraViewExplanation = "" +
        "Camera Axis is the pivot of the camera and Camera Position is the parent where the main " +
        "camera is attached. To adjust the view of the locked camera, only apply position on the X and Z " +
        "values of the Camera Axis transform. Camera position tansform must remain on 0,0,0 position.\n\n" +
        "For rotation, you can adjust the Y Axis of the Camera Axis Transform and the X Axis of the Camera " +
        "Position transform, to adjust the view of such camera\n";
    ...
    void Start ()
    {
        ...
        int lockedCameraListCount = lockedCameraList.Count;

        for (int i = 0; i < lockedCameraListCount; i++) {
            currentCameraElement = lockedCameraList [i];

            int cameraTransformListCount = currentCameraElement.cameraTransformList.Count;

            for (int j = 0; j < cameraTransformListCount; j++) {
    ...
    void checkStartGameWithView ()
    {
        ...
        for (int i = 0; i < lockedCameraListCount; i++) {
            ...
            for (int j = 0; j < cameraTransformListCount; j++) {
                ...
                if (currentCameraAxis.setAIToStartOn2_5DView) {
                    if (currentCameraAxis.setViewOnAllAIOnScene) {
                        findObjectivesSystem [] findObjectivesSystemList = FindObjectsOfType<findObjectivesSystem> ();

                        foreach (findObjectivesSystem currentFindObjectivesSystem in findObjectivesSystemList) {
                            currentFindObjectivesSystem.enableOrDisable2_5ModeOnAIWithPivotPositionValues (true,
                                currentCameraAxis.originalLockedCameraPivotPosition, currentCameraAxis.moveInXAxisOn2_5d);
                        }
                    } else {
                        int AIForViewListCount = currentCameraAxis.AIForViewList.Count;

                        for (int k = 0; k < AIForViewListCount; k++) {
                            if (currentCameraAxis.AIForViewList [k] != null) {
                                findObjectivesSystem currentFindObjectivesSystem = currentCameraAxis.AIForViewList [k].GetComponentInChildren<findObjectivesSystem> ();

                                if (currentFindObjectivesSystem != null) {
                                    currentFindObjectivesSystem.enableOrDisable2_5ModeOnAIWithPivotPositionValues (true, 
                                        currentCameraAxis.cameraPosition.position, currentCameraAxis.moveInXAxisOn2_5d);
                                }
                            }
                        }
                    }
    ...
    void duplicateCameraAxisValues (cameraAxis newState, cameraAxis stateToDuplicate)
    {
        newState.useDifferentCameraFov = stateToDuplicate.useDifferentCameraFov;
        newState.fovValue = stateToDuplicate.fovValue;

        newState.followPlayerPosition = stateToDuplicate.followPlayerPosition;
        newState.putCameraOutsideOfPivot = stateToDuplicate.putCameraOutsideOfPivot;
        ...
        newState.horizontalFollowPlayerPositionSpeed = stateToDuplicate.horizontalFollowPlayerPositionSpeed;

        newState.lookAtPlayerPosition = stateToDuplicate.lookAtPlayerPosition;
        newState.lookAtPlayerPositionSpeed = stateToDuplicate.lookAtPlayerPositionSpeed;

        newState.lookAtCustomTarget = stateToDuplicate.lookAtCustomTarget;

        newState.lookAtCustomMultipleTargets = stateToDuplicate.lookAtCustomMultipleTargets;

        newState.useRotationLimits = stateToDuplicate.useRotationLimits;
        newState.rotationLimitsX = stateToDuplicate.rotationLimitsX;
        newState.rotationLimitsY = stateToDuplicate.rotationLimitsY;

        newState.usePositionOffset = stateToDuplicate.usePositionOffset;
        newState.positionOffset = stateToDuplicate.positionOffset;

        newState.smoothCameraTransition = stateToDuplicate.smoothCameraTransition;
        newState.cameraTransitionSpeed = stateToDuplicate.cameraTransitionSpeed;


        newState.smoothTransitionAtStart = stateToDuplicate.smoothTransitionAtStart;
        newState.previoslyUseSmoohtTransitionState = stateToDuplicate.previoslyUseSmoohtTransitionState;

        newState.setAIToStartOn2_5DView = stateToDuplicate.setAIToStartOn2_5DView;

        newState.setViewOnAllAIOnScene = stateToDuplicate.setViewOnAllAIOnScene;

        newState.cameraCanRotate = stateToDuplicate.cameraCanRotate;

        newState.rangeAngleX = stateToDuplicate.rangeAngleX;
        newState.rangeAngleY = stateToDuplicate.rangeAngleY;

        newState.useSpringRotation = stateToDuplicate.useSpringRotation;
        newState.springRotationDelay = stateToDuplicate.springRotationDelay;

        newState.smoothRotation = stateToDuplicate.smoothRotation;
        newState.rotationSpeed = stateToDuplicate.rotationSpeed;

        newState.originalPivotRotationY = stateToDuplicate.originalPivotRotationY;
        newState.originalCameraRotationX = stateToDuplicate.originalCameraRotationX;

        newState.canUseZoom = stateToDuplicate.canUseZoom;
        newState.zoomValue = stateToDuplicate.zoomValue;

        newState.useZoomByMovingCamera = stateToDuplicate.useZoomByMovingCamera;
        newState.zoomCameraOffsets = stateToDuplicate.zoomCameraOffsets;
        newState.zoomByMovingCameraSpeed = stateToDuplicate.zoomByMovingCameraSpeed;
        newState.zoomByMovingCameraAmount = stateToDuplicate.zoomByMovingCameraAmount;
        newState.zoomByMovingCameraDirectlyProportional = stateToDuplicate.zoomByMovingCameraDirectlyProportional;

        newState.canMoveCamera = stateToDuplicate.canMoveCamera;

        newState.moveCameraSpeed = stateToDuplicate.moveCameraSpeed;
        newState.moveCameraLimitsX = stateToDuplicate.moveCameraLimitsX;
        newState.moveCameraLimitsY = stateToDuplicate.moveCameraLimitsY;
        newState.smoothCameraMovement = stateToDuplicate.smoothCameraMovement;
        newState.smoothCameraSpeed = stateToDuplicate.smoothCameraSpeed;
        newState.useSpringMovement = stateToDuplicate.useSpringMovement;
        newState.springMovementDelay = stateToDuplicate.springMovementDelay;
        ...
        newState.originalCameraLocalPosition = stateToDuplicate.originalCameraLocalPosition;

        newState.originalCameraForward = stateToDuplicate.originalCameraForward;

        newState.followFixedCameraPosition = stateToDuplicate.followFixedCameraPosition;

        newState.changeLockedCameraDirectionOnInputReleased = stateToDuplicate.changeLockedCameraDirectionOnInputReleased;

        newState.useWaypoints = stateToDuplicate.useWaypoints;
        newState.useRaycastToPlaceWaypoints = stateToDuplicate.useRaycastToPlaceWaypoints;

        newState.useSpline = stateToDuplicate.useSpline;

        newState.splineAccuracy = stateToDuplicate.splineAccuracy;

        newState.useZeroCameraTransition = stateToDuplicate.useZeroCameraTransition;

        newState.useHorizontalOffsetOnFaceSide = stateToDuplicate.useHorizontalOffsetOnFaceSide;
        newState.horizontalOffsetOnFaceSide = stateToDuplicate.horizontalOffsetOnFaceSide;
        newState.inputToleranceOnFaceSide = stateToDuplicate.inputToleranceOnFaceSide;
        newState.horizontalOffsetOnFaceSideSpeed = stateToDuplicate.horizontalOffsetOnFaceSideSpeed;

        newState.useHorizontalOffsetOnFaceSideOnMoving = stateToDuplicate.useHorizontalOffsetOnFaceSideOnMoving;
        newState.horizontalOffsetOnFaceSideOnMoving = stateToDuplicate.horizontalOffsetOnFaceSideOnMoving;
        newState.inputToleranceOnFaceSideOnMoving = stateToDuplicate.inputToleranceOnFaceSideOnMoving;
        newState.horizontalOffsetOnFaceSideOnMovingSpeed = stateToDuplicate.horizontalOffsetOnFaceSideOnMovingSpeed;

        newState.useBoundToFollowPlayer = stateToDuplicate.useBoundToFollowPlayer;
        newState.heightBoundTop = stateToDuplicate.heightBoundTop;
        newState.widthBoundRight = stateToDuplicate.widthBoundRight;
        newState.widthBoundLeft = stateToDuplicate.widthBoundLeft;
        newState.depthBoundFront = stateToDuplicate.depthBoundFront;
        newState.depthBoundBackward = stateToDuplicate.depthBoundBackward;
        newState.boundOffset = stateToDuplicate.boundOffset;

        newState.useTankControls = stateToDuplicate.useTankControls;

        newState.playerCanMoveOnAimInTankMode = stateToDuplicate.playerCanMoveOnAimInTankMode;

        newState.use2_5dView = stateToDuplicate.use2_5dView;

        newState.useDefaultZValue2_5d = stateToDuplicate.useDefaultZValue2_5d;
        newState.moveInXAxisOn2_5d = stateToDuplicate.moveInXAxisOn2_5d;

        newState.reverseMovementDirection = stateToDuplicate.reverseMovementDirection;

        newState.moveCameraLimitsX2_5d = stateToDuplicate.moveCameraLimitsX2_5d;
        newState.moveCameraLimitsY2_5d = stateToDuplicate.moveCameraLimitsY2_5d;

        newState.use2_5dVerticalOffsetOnMove = stateToDuplicate.use2_5dVerticalOffsetOnMove;
        newState.verticalTopOffsetOnMove = stateToDuplicate.verticalTopOffsetOnMove;
        newState.verticalBottomOffsetOnMove = stateToDuplicate.verticalBottomOffsetOnMove;
        newState.verticalOffsetOnMoveSpeed = stateToDuplicate.verticalOffsetOnMoveSpeed;

        newState.clampAimDirections = stateToDuplicate.clampAimDirections;
        newState.numberOfAimDirections = stateToDuplicate.numberOfAimDirections;

        newState.useCircleCameraLimit2_5d = stateToDuplicate.useCircleCameraLimit2_5d;
        newState.circleCameraLimit2_5d = stateToDuplicate.circleCameraLimit2_5d;
        newState.scaleCircleCameraLimitWithDistanceToCamera2_5d = stateToDuplicate.scaleCircleCameraLimitWithDistanceToCamera2_5d;
        newState.circleCameraLimitScaleClamp2_5d = stateToDuplicate.circleCameraLimitScaleClamp2_5d;

        newState.adjustPlayerPositionToFixed2_5dPosition = stateToDuplicate.adjustPlayerPositionToFixed2_5dPosition;

        newState.moveReticleInFixedCircleRotation2_5d = stateToDuplicate.moveReticleInFixedCircleRotation2_5d;

        newState.useTopDownView = stateToDuplicate.useTopDownView;

        newState.extraTopDownYRotation = stateToDuplicate.extraTopDownYRotation;

        newState.moveCameraLimitsXTopDown = stateToDuplicate.moveCameraLimitsXTopDown;
        newState.moveCameraLimitsYTopDown = stateToDuplicate.moveCameraLimitsYTopDown;
        newState.use8DiagonalAim = stateToDuplicate.use8DiagonalAim;
        newState.useRelativeMovementToLockedCamera = stateToDuplicate.useRelativeMovementToLockedCamera;

        newState.useCircleCameraLimitTopDown = stateToDuplicate.useCircleCameraLimitTopDown;
        newState.circleCameraLimitTopDown = stateToDuplicate.circleCameraLimitTopDown;
        newState.scaleCircleCameraLimitWithDistanceToCamera = stateToDuplicate.scaleCircleCameraLimitWithDistanceToCamera;
        newState.circleCameraLimitScaleClamp = stateToDuplicate.circleCameraLimitScaleClamp;
        newState.moveReticleInFixedCircleRotationTopDown = stateToDuplicate.moveReticleInFixedCircleRotationTopDown;

        newState.changeRootMotionActive = stateToDuplicate.changeRootMotionActive;
        newState.useRootMotionActive = stateToDuplicate.useRootMotionActive;

        newState.canRotateCameraHorizontally = stateToDuplicate.canRotateCameraHorizontally;
        newState.horizontalCameraRotationSpeed = stateToDuplicate.horizontalCameraRotationSpeed;
        newState.useMouseInputToRotateCameraHorizontally = stateToDuplicate.useMouseInputToRotateCameraHorizontally;
        newState.useFixedRotationAmount = stateToDuplicate.useFixedRotationAmount;
        newState.fixedRotationAmountToRight = stateToDuplicate.fixedRotationAmountToRight;
        newState.fixedRotationAmountToLeft = stateToDuplicate.fixedRotationAmountToLeft;
        newState.fixedRotationAmountSpeed = stateToDuplicate.fixedRotationAmountSpeed;

        newState.useLockedCameraRotationInputToggle = stateToDuplicate.useLockedCameraRotationInputToggle;

        newState.ignoreCameraRotationToLeftAndRightInput = stateToDuplicate.ignoreCameraRotationToLeftAndRightInput;

        newState.checkPossibleTargetsBelowCursor = stateToDuplicate.checkPossibleTargetsBelowCursor;
        ...
        newState.reticleOffset = stateToDuplicate.reticleOffset;

        newState.useAimAssist = stateToDuplicate.useAimAssist;
        newState.lookOnlyIfTargetOnScreen = stateToDuplicate.lookOnlyIfTargetOnScreen;
        newState.lookOnlyIfTargetVisible = stateToDuplicate.lookOnlyIfTargetVisible;
        newState.moveCameraCursorSpeed = stateToDuplicate.moveCameraCursorSpeed;

        newState.canUseManualLockOn = stateToDuplicate.canUseManualLockOn;

        newState.setDeactivateRootMotionOnStrafeActiveOnLockedViewValue = stateToDuplicate.setDeactivateRootMotionOnStrafeActiveOnLockedViewValue;
        newState.deactivateRootMotionOnStrafeActiveOnLockedView = stateToDuplicate.deactivateRootMotionOnStrafeActiveOnLockedView;

        newState.useOldSchoolAim = stateToDuplicate.useOldSchoolAim;
        newState.aimOffsetUp = stateToDuplicate.aimOffsetUp;
        newState.aimOffsetDown = stateToDuplicate.aimOffsetDown;
        newState.aimOffsetChangeSpeed = stateToDuplicate.aimOffsetChangeSpeed;

        newState.setCanMoveWhileAimLockedCameraState = stateToDuplicate.setCanMoveWhileAimLockedCameraState;
        newState.canMoveWhileAimLockedCamera = stateToDuplicate.canMoveWhileAimLockedCamera;

        newState.usePointAndClickSystem = stateToDuplicate.usePointAndClickSystem;

        newState.useTransparetSurfaceSystem = stateToDuplicate.useTransparetSurfaceSystem;

        newState.useDistanceToTransformToMoveCameraCloserOrFarther = stateToDuplicate.useDistanceToTransformToMoveCameraCloserOrFarther;

        newState.moveCameraCloserOrFartherSpeed = stateToDuplicate.moveCameraCloserOrFartherSpeed;
        newState.useDistanceDirectlyProportional = stateToDuplicate.useDistanceDirectlyProportional;

        newState.useClampCameraForwardDirection = stateToDuplicate.useClampCameraForwardDirection;

        newState.useClampCameraBackwardDirection = stateToDuplicate.useClampCameraBackwardDirection;
        newState.clampCameraBackwardDirection = stateToDuplicate.clampCameraBackwardDirection;

        newState.cameraDistanceMultiplier = stateToDuplicate.cameraDistanceMultiplier;

        newState.useCameraLimitSystem = stateToDuplicate.useCameraLimitSystem;
        newState.mainCamera2_5dZoneLimitSystem = stateToDuplicate.mainCamera2_5dZoneLimitSystem;
        newState.disablePreviousCameraLimitSystem = stateToDuplicate.disablePreviousCameraLimitSystem;

        newState.canChangeCameraToFreeViewByInput = stateToDuplicate.canChangeCameraToFreeViewByInput;

        newState.rotatePlayerToward2dCameraOnTriggerEnter = stateToDuplicate.rotatePlayerToward2dCameraOnTriggerEnter;
    }
    ...
    public void duplicateLockedCamera (int lockedCameraListIndex, int cameraTransformListIndex)
    {
        ...
        duplicateCameraAxisValues (currentCameraElement, currentCameraElementToDuplicate);
    ...
    void updateComponent ()
    {
        ...
        GKC_Utils.updateDirtyScene ("Update Locked Camera System", gameObject);
    }
    ...
    public class cameraAxis
    {
        ...
        public bool setAIToStartOn2_5DView;
        public List<GameObject> AIForViewList = new List<GameObject> ();
        public bool setViewOnAllAIOnScene;
        ...
        public bool useLockedCameraRotationInputToggle;
        public bool ignoreCameraRotationToLeftAndRightInput;
